...
const load = async () => {
  const { items, total: t } = await CategoryAPI.list({
    page: page.value,
    limit: limit.value,
    search: search.value,
  });
...
const onSave = async (payload) => {
  if (editing.value) await CategoryAPI.update(editing.value._id, payload);
...
<template>
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
      <h2 class="text-3xl font-semibold text-gray-800">📂 จัดการหมวดหมู่</h2>
      <button
        @click="
          editing = null;
          showModal = true;
      ...
      <table class="min-w-full divide-y divide-gray-200 text-sm">
        <thead class="bg-gray-100 text-gray-700 uppercase">
          <tr>
            ...
            <th class="px-6 py-3 text-left">คำอธิบาย</th>
    ...
    <transition name="fade">
      ...
      >
        ...
        >
          ...
          <CategoryForm :model-value="editing" @submit="onSave" />
